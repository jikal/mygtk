#include <gtk/gtk.h>
inline char*_(char* c)   //用于显示中文字体,默认Windows下Gtk显示中文乱码,使用次函数转换编码
{
...
void gameover(gboolean won)
{
     ...
     g_snprintf(msg, 100, _("哇！你这么牛，用时 %3d 秒。"), game_time);
          ...
          { 
     g_snprintf(msg, 100, _("太有才了！这么快就玩完了～～"));
     }
        for(i=0, index=0; i<height; i++)//游戏结束后让所有格子都打开
                     ...
                     if(map[index].opened==FALSE)
                     {
                        ...
                        gtk_button_set_label(GTK_BUTTON(map[index].button), " ");
                         if(map[index].mine==FALSE) 
                          {
                             if(map[index].count==0) gtk_button_set_label(GTK_BUTTON(map
...
[index].button), "");
                        ...
                        else gtk_button_set_label(GTK_BUTTON(map[index].button), _("雷"));
                       }
                  index++;
               }
         }
...
for(i=0, index=0; i<height; i++)//所有格子的初始化
